...
public class ClanLeaveCmd extends Command {
    public ClanLeaveCmd(String command, String usage, String[] aliases, String description, CommandSubsystem subsystem) {
    ...
    public void execute(String[] args, Guild guild, Member sender, TextChannel channel, Message msg) {
        ...
        if (clan.getLeader() == player) {
            Embed reply = new Embed(EmbedType.ERROR, "Error", Msg.getMsg("clan-leader"), 1);
            msg.replyEmbeds(reply.build()).queue();
            return;
        }

        Embed reply = new Embed(EmbedType.SUCCESS, "", Msg.getMsg("clan-left"), 1);
        msg.replyEmbeds(reply.build()).queue();
    }
}
